...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
  SELECT DISTINCT
    f.num, f.company, f.transfer, s.num, s.company
  FROM
    (SELECT
       stopb.name transfer, stopb.id transfer_id, a.company company, a.num num
     FROM
       routes a
     JOIN
       routes b ON (a.company = b.company AND a.num = b.num)
     ...
     JOIN
       stops stopb ON b.stop_id = stopb.id
     WHERE
       stopa.name = 'Craiglockhart') f
  JOIN
    (SELECT
      stopa.id transfer_id, a.company company, a.num num
    FROM
      routes a
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
    ...
    JOIN
      stops stopb ON b.stop_id = stopb.id
    WHERE
      stopb.name = 'Sighthill') s ON f.transfer_id = s.transfer_id
  SQL
end
